...
- **Documented, not code-changed (with reason):** H-1 + M-18 (console/server `state` blocker); RB-1 (default-on binding = product/ADR decision); RB-2 (needs a server read-only existence endpoint). M-9 was in this list until 2026-09-11 — see its entry below.
...
### Config / supply-chain
- **M-9 · `INDEXER_API_KEY` bundled into the shipped app — `FIXED 2026-09-11`.** `tauri.conf.json` ships `.env` as a bundle resource, and the key was extractable from every distributed build. Resolved by removing the key rather than scoping it: the indexer accepts the user's own Hippius session token, resolves it to an SS58, and binds the routes that carry account records to that account. `.env` is still bundled, so `scripts/write-tauri-env.sh` now **allowlists** what goes into it — the app no longer reading `INDEXER_API_KEY` would not have stopped the release workflows from copying it out of the `TAURI_ENV_FILE` secret and into every installer, which is the exact shape of this finding. See hippius-indexer `docs/plans/2026-09-11-indexer-user-token-auth-design.md`; mobile-app had the same exposure through `EXPO_PUBLIC_INDEXER_API_KEY` and was fixed alongside.
