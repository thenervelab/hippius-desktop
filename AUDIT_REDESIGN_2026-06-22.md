...
### Config / supply-chain
- **M-9 · `INDEXER_API_KEY` bundled into the shipped app — `FIXED 2026-09-11`.** `tauri.conf.json` ships `.env` as a bundle resource, and the key was extractable from every distributed build. Resolved by removing the key rather than scoping it: the indexer accepts the user's own Hippius session token, resolves it to an SS58, and binds the routes that carry account records to that account. `.env` is still bundled, but everything in it is now optional. See hippius-indexer `docs/plans/2026-09-11-indexer-user-token-auth-design.md`; mobile-app had the same exposure through `EXPO_PUBLIC_INDEXER_API_KEY` and was fixed alongside.
