...
- **Documented, not code-changed (with reason):** H-1 + M-18 (console/server `state` blocker); M-9 (INDEXER_API_KEY is runtime-read from the bundled resource → server-side key scoping, architectural); RB-1 (default-on binding = product/ADR decision); RB-2 (needs a server read-only existence endpoint); NOTIF-4 + tray rect-validation + a few FE-lows (tracked lows).
...
- **Bridge → Rust port (H-8 Part B):** the funds-critical + security parts are DONE. New `blockchain/bridge/` module (subxt codegen for both chains, conversion/fee math, chain-correct ink! error decode, two-chain client, both **deposit + withdraw** write paths, read quotes) — all compile-verified, pure parts unit-tested. The FE writes now `invoke` the Rust commands, and **`local_wallet_sign` (the blind-sign command) is deleted** (zero remaining callers) — H-8's root is closed. ⚠️ The on-chain write paths are compile-verified only and MUST be smoke-tested on a funded Bittensor/Hippius testnet wallet before release (see `bridge/DEPOSIT_PORT_NOTES.md`). **Remaining (Low, non-funds):** the bridge *read* paths (balances/hotkeys/tx-history) still use the TS service; porting them to Rust + deleting `app/lib/bridge/*` + removing the polkadot-api deps is the leftover logic-placement cleanup.
