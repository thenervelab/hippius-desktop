// tests/hippius_restore_from_remote_no_conflict_update.rs
...
async fn assert_singleton_no_conflict(
    ...
    for ctx in [a, b] {
        ...
        anyhow::ensure!(
            ...
            "[RFR] {} must not create conflict siblings for {}; got {:?}",
...
#[test]
fn restore_from_remote_no_conflict_on_shared_update() -> Result<()> {
    let os_stack = std::env::var("SMOKE_OS_STACK_BYTES")
        .ok().and_then(|v| v.parse().ok())
        .unwrap_or(16 * 1024 * 1024);

    let jh = std::thread::Builder::new()
        .name("hippius_restore_from_remote_no_conflict_update".into())
        .stack_size(os_stack)
        .spawn(|| -> Result<()> {
            ...
            rt.block_on(async {
                ...
                let (a_ctx, _ta) = make_client_ctx(
                    "A", &boot.s3_endpoint, &bucket, format!("{tag8}-A"), harness::DeletePolicy::RestoreFromRemote
                ).await?;
                let (b_ctx, _tb) = make_client_ctx(
                    "B", &boot.s3_endpoint, &bucket, format!("{tag8}-B"), harness::DeletePolicy::RestoreFromRemote
                ).await?;

                // Seed only on A -> both should adopt under RFR
                let key = format!("pair/noconf-rfr-{}.txt", Uuid::new_v4().simple());
                write_one(&a_ctx.local_root, &key, "seed-rfr\n").await?;
                sync_until_present_on_both(&a_ctx, &b_ctx, &[key.clone()], check_iters).await?;

                // A updates -> B must adopt (without creating conflict)
                ...
                assert!(a_body.contains("updated-by-A") && b_body.contains("updated-by-A"),
                        "[RFR] both clients must adopt updated body for {}", key);

                let r_body = read_remote_text(&a_ctx.s3, &bucket, &key).await?;
                assert!(r_body.contains("updated-by-A"), "[RFR] remote must reflect updated body");

                // No duplicates / conflict siblings anywhere for the A->B update.
                assert_singleton_no_conflict(&a_ctx, &b_ctx, &bucket, &key).await?;

                // B updates -> A must adopt (also without creating conflict)
                ...
                assert!(
                    ...
                    "[RFR] both clients must adopt B's updated body for {}",
                    key
                );
                ...
                assert!(
                    ...
                    "[RFR] remote must reflect B's updated body"
                );

                // No duplicates / conflict siblings anywhere for the B->A update either.
                assert_singleton_no_conflict(&a_ctx, &b_ctx, &bucket, &key).await?;
