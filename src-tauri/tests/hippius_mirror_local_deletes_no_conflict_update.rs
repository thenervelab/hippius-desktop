// tests/hippius_mirror_local_deletes_no_conflict_update.rs
...
async fn assert_singleton_no_conflict(
    ...
    for ctx in [a, b] {
        ...
        anyhow::ensure!(
            ...
            "[MirrorLocalDeletes] {} must not create conflict siblings for {}; got {:?}",
            ctx.name,
            key,
            siblings
        );
...
#[test]
fn mirror_local_deletes_no_conflict_on_shared_update() -> Result<()> {
    ...
    let jh = std::thread::Builder::new()
        .name("hippius_mld_no_conflict_update".into())
        .stack_size(os_stack)
        .spawn(|| -> Result<()> {
            ...
            rt.block_on(async {
                ...
                let (a_ctx, _ta) = make_client_ctx(
                    ...
                    &bucket,
                    format!("{tag8}-A"),
                    harness::DeletePolicy::MirrorLocalDeletes,
                )
                .await?;
                ...
                // Seed via A -> both adopt under MLD
                let key = format!("pair/noconf-ml-{}.txt", Uuid::new_v4().simple());
                write_one(&a_ctx.local_root, &key, "seed-ml\n").await?;
                sync_until_present_on_both(&a_ctx, &b_ctx, &[key.clone()], check_iters).await?;

                // A updates -> both must adopt, no conflict
                ...
                assert!(
                    a_body.contains("updated-by-A") && b_body.contains("updated-by-A"),
                    "[MLD] both clients must adopt updated body for {}",
                    key
                );
                let r_body = read_remote_text(&a_ctx.s3, &bucket, &key).await?;
                assert!(
                    r_body.contains("updated-by-A"),
                    "[MLD] remote must reflect updated body"
                );

                // No duplicate keys or conflict siblings for the A->B update.
                assert_singleton_no_conflict(&a_ctx, &b_ctx, &bucket, &key).await?;

                // B updates -> both must adopt, still no conflict
                ...
                assert!(
                    ...
                    "[MLD] both clients must adopt updated body for {} (B->A direction)",
                    key
                );
                let r_body2 = read_remote_text(&a_ctx.s3, &bucket, &key).await?;
                assert!(
                    r_body2.contains("updated-by-B"),
                    "[MLD] remote must reflect B's updated body"
                );

                // Still no duplicate keys or conflict siblings after the B->A update.
                assert_singleton_no_conflict(&a_ctx, &b_ctx, &bucket, &key).await?;
