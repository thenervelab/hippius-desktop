...
///
/// One constant so the status emit, the persisted notification copy (the
/// revoked arm in `sync::tauri_bridge` rewrites its `SYNC_FAILED_NOTIFY`
/// payload's `error` to this before emitting — the raw engine marker stays
/// on `SYNC_ERROR` only), and the FE tests cannot drift apart. Deliberately
/// terminal wording: the drive is not "paused" or "failing", access is gone
/// (the owner removed this member or deleted the drive) and the only
/// affordance left is Remove.
