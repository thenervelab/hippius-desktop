...
impl RateLimitState {
    ...
    ///
    /// The reservation (`in_flight`) is what closes the TOCTOU window — see the
    /// field doc on `State::in_flight`. The threshold is evaluated against
    /// `failures_in_window + in_flight`, so concurrent attempts that have
    /// passed `check` but not yet recorded a result still count.
