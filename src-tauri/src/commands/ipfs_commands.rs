...
pub async fn encrypt_and_upload_file(
    account_id: String,
    file_name: String,   // Now passed directly
    file_data: Vec<u8>,  // File content as bytes
    seed_phrase: String,
    encryption_key: Option<Vec<u8>>,
) -> Result<String, String> {
    println!("Processing file: {} ({} bytes)", file_name, file_data.len());    
    ...
    // Create temp directory (auto-deletes when dropped)
    let temp_dir = tempfile::tempdir().map_err(|e| e.to_string())?;
    let temp_dir_path = temp_dir.path().to_path_buf();

    let api_url_cloned = api_url.to_string();
    let encryption_key_cloned = encryption_key.clone();
    let file_data_cloned = file_data.clone(); // Clone file_data for use in closure
    ...
    let (file_name, metadata_cid, chunk_pairs) = tokio::task::spawn_blocking(move || {
        ...
        let original_size = file_data_cloned.len();

        // Encrypt the file data
        let to_process = if let Some(key) = &encryption_key_cloned {
            // Replace with your actual encryption logic
            let mut encrypted = file_data_cloned.clone();
            for (i, byte) in encrypted.iter_mut().enumerate() {
                *byte ^= key[i % key.len()]; // Simple XOR example
            }
            encrypted
        } else {
            file_data_cloned.clone()
        };
        let encrypted_size = to_process.len();

        // Split into chunks for erasure coding
        let mut chunks = Vec::new();
        for i in (0..to_process.len()).step_by(chunk_size) {
            let end = std::cmp::min(i + chunk_size, to_process.len());
            let mut chunk = to_process[i..end].to_vec();
        ...
        let mut all_chunk_info = Vec::new();
        ...
        for (chunk_index, chunk) in chunks.iter().enumerate() {
            ...
            for (share_idx, shard) in shard_refs.iter().enumerate() {
                let chunk_name = format!("{}_chunk_{}_{}.ec", file_id, chunk_index, share_idx);
                let chunk_path = temp_dir_path.join(&chunk_name);
                fs::write(&chunk_path, shard).map_err(|e| e.to_string())?;
                ...
                all_chunk_info.push(ChunkInfo {
                    ...
                    original_chunk: chunk_index,
                    share_idx,
                    size: shard.len(),
                });
                chunk_pairs.push((chunk_name, cid));
            }
        }
        ...
        let metadata_path = temp_dir_path.join(format!("{}_metadata.json", file_id));
        fs::write(&metadata_path, serde_json::to_vec(&metadata).map_err(|e| e.to_string())?)
            .map_err(|e| e.to_string())?;
        
        let metadata_cid = upload_to_ipfs(&api_url_cloned, metadata_path.to_str().unwrap())
            .map_err(|e| e.to_string())?;
        
        Ok::<_, String>((file_name, metadata_cid, chunk_pairs))
    })
    .await
    .map_err(|e| e.to_string())??;
    ...
    let mut files_for_storage = vec![(meta_filename.clone(), metadata_cid.clone())];
    files_for_storage.extend(chunk_pairs);

    match request_erasure_storage(&meta_filename, &files_for_storage, api_url, &seed_phrase).await {
        Ok(res) => {
            // Create temporary file for sync folder
            let temp_file = tempfile::NamedTempFile::new().map_err(|e| e.to_string())?;
            fs::write(temp_file.path(), &file_data).map_err(|e| e.to_string())?;
            ...
            println!("[encrypt_and_upload_file] Success: {}", res);
            Ok(metadata_cid)
        },
        Err(e) => {
            println!("[encrypt_and_upload_file] Storage error: {}", e);
            Err(e)
        }
    }
}
...
pub async fn upload_file_public(
    ...
    use std::path::Path;
    println!("[upload_file_public] uploading file: {} ({} bytes)", file_name, file_data.len());    
    let api_url = "http://127.0.0.1:5001";
    ...
    let api_url_cloned = api_url.to_string();
    let temp_file_path_str = temp_file_path.to_string_lossy().to_string(); // Convert to owned String
    
    let file_cid = tokio::task::spawn_blocking(move || {
        upload_to_ipfs(&api_url_cloned, &temp_file_path_str)
    })
    .await
    ...
    // Create another temp file for sync folder copy
    let sync_temp_file = tempfile::NamedTempFile::new()
        .map_err(|e| format!("Temp file creation error: {e}"))?;
    std::fs::write(sync_temp_file.path(), &file_data)
        .map_err(|e| format!("Failed to write sync temp file: {e}"))?;

    // Call request_file_storage
...
pub async fn public_upload_with_erasure(
    ...
    println!("[public_upload_with_erasure] uploading file: {} ({} bytes)", file_name, file_data.len());    
    let api_url = "http://127.0.0.1:5001";
    let k = DEFAULT_K;
    let m = DEFAULT_M;
    let chunk_size = DEFAULT_CHUNK_SIZE;
    ...
    let (file_name, metadata_cid, _original_file_cid, chunk_pairs) = tokio::task::spawn_blocking(move || {
        // Calculate original file hash
        let mut hasher = Sha256::new();
        hasher.update(&file_data_cloned);
        let original_file_hash = format!("{:x}", hasher.finalize());

        // Write original file to temp location and upload
        let original_temp_path = temp_dir_path.join("original_file");
        fs::write(&original_temp_path, &file_data_cloned).map_err(|e| e.to_string())?;
        let original_file_cid = upload_to_ipfs(&api_url_cloned, original_temp_path.to_str().unwrap())
            .map_err(|e| e.to_string())?;

        // Process file data directly (no encryption for public uploads)
        let to_process = file_data_cloned;
        ...
        Ok::<(String, String, String, Vec<(String, String)>), String>((file_name, metadata_cid, original_file_cid, chunk_pairs))
    })
    .await
    .map_err(|e| e.to_string())??;

    // Build files array: original file + metadata + chunks
    let meta_filename = format!("{}{}", file_name, if file_name.ends_with(".ec_metadata") { "" } else { ".ec_metadata" });
    let mut files_for_storage = Vec::with_capacity(chunk_pairs.len() + 2);
    files_for_storage.push((meta_filename.clone(), metadata_cid.clone())); // Metadata CID
    files_for_storage.extend(chunk_pairs); // Chunk CIDs

    // Create temp file for sync folder copy
    let sync_temp_file = tempfile::NamedTempFile::new()
        .map_err(|e| format!("Temp file creation error: {e}"))?;
    fs::write(sync_temp_file.path(), &file_data)
        .map_err(|e| format!("Failed to write sync temp file: {e}"))?;
...
pub async fn encrypt_and_upload_folder(
    ...
    let api_url_cloned = api_url.to_string();
    let encryption_key_cloned = encryption_key.clone();
    let files_cloned = files.clone(); // Clone files for use in closure
    ...
    let (folder_name, folder_metadata_cid, all_files_for_storage) = tokio::task::spawn_blocking(move || {
        ...
        for (relative_path, file_data) in files_cloned {
            // Calculate file hash
            let mut hasher = Sha256::new();
            ...
            let meta_filename = format!(
                "{}{}",
                file_name,
                if file_name.ends_with(".ec_metadata") { "" } else { ".ec_metadata" }
            );
            all_files_for_storage.push((meta_filename, metadata_cid));
            all_files_for_storage.extend(chunk_pairs);
        }

        // Create and upload folder metadata
        let folder_metadata_path = temp_dir.path().join("folder_metadata.json");
        fs::write(&folder_metadata_path, serde_json::to_vec(&file_entries).map_err(|e| e.to_string())?)
            .map_err(|e| e.to_string())?;
        let folder_metadata_cid = upload_to_ipfs(&api_url_cloned, folder_metadata_path.to_str().unwrap())
            .map_err(|e| e.to_string())?;

        // Add folder metadata to storage list
        ...
        Ok::<_, String>((folder_name, folder_metadata_cid, all_files_for_storage))
    })
    .await
    .map_err(|e| e.to_string())??;
    ...
    match storage_result {
        Ok(res) => {
            // Create temp folder structure for sync
            let temp_folder = tempfile::tempdir().map_err(|e| e.to_string())?;
            for (relative_path, file_data) in files {
                let file_path = temp_folder.path().join(&relative_path);
...
pub async fn public_upload_folder(
    ...
    let (folder_name, folder_metadata_cid, file_cid_pairs) = tokio::task::spawn_blocking(move || {
        ...
        for (relative_path, file_data) in files_cloned {
            ...
            // Write file to temp location and upload
            let temp_file_path = temp_dir.path().join(&file_name);
            fs::write(&temp_file_path, &file_data).map_err(|e| e.to_string())?;
            let file_cid = upload_to_ipfs(&api_url_cloned, temp_file_path.to_str().unwrap())
                .map_err(|e| e.to_string())?;
            ...
            file_cid_pairs.push((ipfs_file_name, file_cid));
        }
        ...
        Ok::<_, String>((folder_name, folder_metadata_cid, file_cid_pairs))
    })
    .await
    .map_err(|e| e.to_string())??;

    // Prepare files for storage request
    ...
    let storage_result = request_folder_storage(&meta_folder_name, &files_for_storage, api_url, &seed_phrase).await;
    match storage_result {
        Ok(res) => {
            // Create temp folder structure for sync
            let temp_folder = tempfile::tempdir().map_err(|e| e.to_string())?;
...
// Helper function to sanitize names for local sync
    ...
    let (folder_name, folder_metadata_cid, all_files_for_storage) = tokio::task::spawn_blocking(move || {
        ...
        for file_path in files {
            let relative_path = file_path
                .strip_prefix(&folder_path_cloned)
                .map_err(|e| e.to_string())?
                .to_string_lossy()
                .to_string();
        ...
        Ok::<(String, String, Vec<(String, String)>), String>((folder_name, folder_metadata_cid, all_files_for_storage))
    })
    .await
    .map_err(|e| e.to_string())??;
...
pub async fn public_upload_folder_sync(
    ...
    let (folder_name, folder_metadata_cid, file_cid_pairs) = tokio::task::spawn_blocking(move || {
        let mut file_entries = Vec::new();
        let mut files = Vec::new();
        let mut file_cid_pairs: Vec<(String, String)> = Vec::new(); // (relative_path, file_cid)
        let _ = collect_files_recursively(&folder_path_cloned, &mut files);
        let temp_dir = tempdir().map_err(|e| e.to_string())?;
        for file_path in files {
