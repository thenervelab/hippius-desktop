...
use crate::{
    ...
    constants::folder_sync::SyncStatusResponse,
    sync_engine::{DeletePolicy, prunefile_id, sync_once_cas},
    utils::{
        fs_watcher::{FsEvent, FsWatcher},
        sync::get_private_sync_path,
    },
};

pub use crate::sync_shared::{
    BucketItem, GLOBAL_CANCEL_TOKEN, S3_PRIVATE_SYNC_STATE, SYNCING_ACCOUNTS, folder_size,
    insert_bucket_item_if_absent,
};
use crate::sync_shared::{MAX_RECENT_ITEMS, RecentItem};

/// Single-flight sync trigger with coalescing.
...
/// Ensure bucket exists (HEAD → CREATE if 404). Private: no public-read policy applied.
async fn ensure_bucket(client: &Client, bucket: &str) {
    match client.head_bucket().bucket(bucket).send().await {
        Ok(_) => {
            println!("Found existing private bucket");
            return;
        }
        Err(_) => {
            ...
            match result {
                Ok(_) => {
                    println!("Successfully created private bucket");
                }
                Err(result) => {
                    println!("Failed to create private bucket: {}", result);
                }
            }
        }
    }
...
/// File-system watcher pipeline: batches FS events and updates local state + UI,
/// then *triggers* a single-flight sync.
...
pub async fn start_private_folder_sync(
    ...
    // First check if we have a valid sync path
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        match activate_bookmark(&sync_path).await {
            ...
            Ok(false) | Err(_) => {
                eprintln!(
                    "[PrivateFolderSync] No bookmark found. Will attempt to create one after filesystem access."
                );
    ...
    if needs_bookmark_creation {
        ...
        println!("[PrivateFolderSync] Attempting to create bookmark after successful FS access...");
        if let Err(e) = store_bookmark(&sync_path, "private").await {
            eprintln!("[PrivateFolderSync] Failed to create bookmark: {}", e);
        } else {
             // Try activating the newly created bookmark to ensure it works
            match activate_bookmark(&sync_path).await {
                Ok(true) => println!("[PrivateFolderSync] Successfully created and activated bookmark"),
                _ => eprintln!("[PrivateFolderSync] Failed to activate newly created bookmark"),
            }
        }
    }
    ...
    let prunefile_id = prunefile_id(&sub_account, &path_hash, "private");
    ...
    // NEW: single-flight trigger + FS pipeline
    ...
    loop {
        ...
        let status = match &result {
            Ok(()) => {
                ...
                let mut st = S3_PRIVATE_SYNC_STATE.lock().unwrap();
            ...
            Err(e) => {
                ...
                eprintln!("[PrivateFolderSync] sync_once_cas failed: {e:#}");
...
async fn process_batch(events: &[FsEvent], pool: &SqlitePool, owner: &str, bucket_name: &str) {
        ...
        match event {
            ...
            FsEvent::Remove(path, is_dir) => {
                ...
                match crate::commands::syncing::get_sync_policy_from_db().await {
                    ...
                    {
                        if let Ok(sync_root) = crate::utils::sync::get_private_sync_path().await {
                                        ...
                                        rel.components().next().and_then(|c| c.as_os_str().to_str())
                                    {
                                        if let Err(e) =
                                            crate::sync_shared::delete_bucket_item_by_name(
                                                pool, owner, "private", top,
                                            )
                                            .await
                                        {
                                            eprintln!(
                                                "[PrivateFolderSync] DB delete failed for '{}': {}",
                                                top, e
                                            );
                                        } else {
                                            println!(
                                                "[PrivateFolderSync] Deleted '{}' from database due to MirrorLocalDeletes policy",
                                                top
                                            );
                                        }
                                    }
                    ...
                    Ok(_) => {
                        ...
                        if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                            println!(
                                "[PrivateFolderSync] Skipping DB delete for '{}' as policy is not MirrorLocalDeletes",
                                file_name
                            );
                        }
                    }
                    Err(e) => {
                        eprintln!("[PrivateFolderSync] Failed to get sync policy: {}", e);
                    }
                }
            }
        }
    ...
    if !bucket_items.is_empty() {
        // Preserve existing behavior: only insert top-level items.
        let sync_path = match get_private_sync_path().await {
            Ok(path) => path,
            Err(e) => {
