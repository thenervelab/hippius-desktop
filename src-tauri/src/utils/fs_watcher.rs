            ...
            EventKind::Modify(notify::event::ModifyKind::Name(rename_type)) => {
                ...
                if paths.len() == 2 {
                    ...
                    if from_in_watched && !to_in_watched {
                        ...
                        info!(
                            "[FsWatcher] Delete detected (moved outside): {:?} (is_dir: {})",
                        ...
                        info!(
                            "[FsWatcher] Create detected (moved inside): {:?} (is_dir: {})",
                        ...
                        info!(
                            "[FsWatcher] Rename within directory: {:?} -> {:?} (is_dir: {})",
                            from_path.file_name().and_then(|n| n.to_str()),
                            to_path.file_name().and_then(|n| n.to_str()),
                            is_dir
                        );
                    ...
                    for path in paths {
                        if path.starts_with(root_dir) {
                            let is_dir = path.is_dir();
                            info!(
                                "[FsWatcher] Fallback handling for: {:?} (is_dir: {})",
                                path.file_name().and_then(|n| n.to_str()),
                                is_dir
                            );
