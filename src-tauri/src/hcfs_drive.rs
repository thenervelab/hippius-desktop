...
pub async fn start_sync_loop(app: AppHandle) {
    ...
    let mut watcher: RecommendedWatcher =
        match notify::recommended_watcher(move |res: Result<notify::Event, notify::Error>| {
            ...
            Ok(w) => w,
            Err(e) => {
                eprintln!("[Sync] Failed to create file watcher: {}. Sync loop will run without file watching.", e);
    ...
    if let Err(e) = watcher.watch(&sync_path, RecursiveMode::Recursive) {
        eprintln!("[Sync] Failed to watch path {:?}: {}. Continuing with heartbeat-only sync.", sync_path, e);
    }
