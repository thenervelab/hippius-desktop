...
import { type MatrixClient, type MatrixEvent, MatrixEventEvent, type Room, RoomEvent, RoomMemberEvent } from "matrix-js-sdk";
import { ArrowDown, Hash, Lock } from "lucide-react";

import { jumpToEventAtom } from "@/components/chat/chat-ui-atoms";
...
import { formatDayLabel, isMessageLike, messageBody } from "@/lib/chat/timeline";
 ...
 */
export default function MessageList({ client, room, summary, searchQuery, timeline }: MessageListProps) {
  ...
  const visibleItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      return body.text.toLowerCase().includes(q) || sender.toLowerCase().includes(q);
    });
  ...
  useEffect(() => {
    ...
    if (added > 0 && !atBottom) {
      const fresh = events.slice(-added).filter((e) => isMessageLike(e) && e.getSender() !== me).length;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const target = byId.get(jump.eventId) ?? (await ensureEventLoaded(client, room, jump.eventId));
      if (cancelled) return;
      if (!target) {
        setJump(null);
        return;
      ...
      for (let i = 0; i < 10 && !byId.has(jump.eventId) && !room.getLiveTimeline().getEvents().some((e) => e.getId() === jump.eventId); i++) {
      ...
      requestAnimationFrame(() => {
        ...
        setAtBottom(false);
        setJump(null);
        window.setTimeout(() => setHighlightId((id) => (id === jump.eventId ? null : id)), 2500);
      });
    })();
    return () => {
      cancelled = true;
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      ...
      >
        ...
        {empty && searchQuery ? (
          <p className="px-4 py-10 text-center text-sm text-grey-60 dark:text-grey-dark-700">No messages match “{searchQuery}”.</p>
        ) : null}
        ...
        {visibleItems.map((item) => {
          switch (item.kind) {
            case "day":
              return (
                <div key={item.key} className="sticky top-0 z-[5] my-2 flex items-center gap-3 px-4" role="separator" aria-label={formatDayLabel(item.ts)}>
            ...
            case "new-line":
              return (
                <div key={item.key} className="my-1 flex items-center gap-2 px-4" role="separator" aria-label="New messages">
                  <span className="h-px flex-1 bg-error-50 dark:bg-error-50" />
                  <span className="text-[11px] font-semibold uppercase tracking-wide text-error-50 dark:text-error-50">New</span>
                </div>
              );
            case "state":
              return (
                <p key={item.key} className="px-4 py-1 pl-16 text-xs text-grey-60 dark:text-grey-dark-700">
                  {item.text}
                </p>
              );
            case "message": {
              ...
              return (
                <MessageRow
                  ...
                  tick={rowTick}
                  highlighted={highlightId !== null && highlightId === item.event.getId()}
      ...
      {!atBottom ? (
        <button
          ...
          className={cn(
            ...
            newBelow > 0 && "border-primary-50 text-primary-50 dark:border-primary-40 dark:text-primary-40",
          )}
        >
  ...
  return (
    <div className="px-4 pb-4 pt-8">
      <div className="mb-3 inline-flex size-12 items-center justify-center rounded-lg bg-grey-90 text-grey-10 dark:bg-black-500 dark:text-grey-light-100">
        {summary.isPublic ? <Hash className="size-6" aria-hidden /> : <Lock className="size-6" aria-hidden />}
      </div>
...
function TypingLine({ client, room }: { client: MatrixClient; room: Room }) {
  ...
  return (
    <p className="h-5 px-4 pl-16 text-xs text-grey-60 dark:text-grey-dark-700" aria-live="polite">
      {text}
    </p>
  );
}
