...
export async function setPublicSyncPath(path: string, accountId?: string): Promise<string> {
  return await invoke<string>("set_sync_path", {
    params: { path, is_public: true, account_id: accountId },
  });
}

export async function setPrivateSyncPath(path: string, accountId?: string): Promise<string> {
  return await invoke<string>("set_sync_path", {
