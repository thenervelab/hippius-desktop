...
export interface SelectProps {
   ...
   * tighter menu (13px labels, 12px descriptions) for selects that sit in a
   * row beside other controls, such as a role picker. Default is the tall
   * form-field select.
   */
  size?: "default" | "compact";
  /**
   * `quiet`: no border, fill or ring until hover or focus, so a select in a
   * list row reads as a value with a caret (a person's role) rather than a
   * form field. Compact only in practice.
   */
  chrome?: "field" | "quiet";
}
        ...
        >
          <SelectPrimitive.Trigger
            ...
            className={cn(
              inputShell,
              compact
                ? "group h-[34px] min-h-0 min-w-0 items-center justify-between gap-1 px-2.5 py-0 text-left shadow-none dark:shadow-none focus-within:shadow-[0px_0px_0px_3px_rgba(49,103,221,0.12)]"
          ...
          >
            <span
              className={cn(
                compact
                  ? "min-w-0 flex-1 truncate text-left text-[13px] font-medium leading-5 tracking-normal"
            ...
            <SelectPrimitive.Icon asChild>
              {isLoading ? (
                ...
                <ChevronDown
                  className={cn(
                    compact
                      ? "size-3.5 shrink-0 text-grey-50 transition-transform duration-200 dark:text-[#7d7d7d]"
          ...
          <SelectPrimitive.Portal>
            <SelectPrimitive.Content
              position="popper"
              sideOffset={4}
              // A compact select sits at the right of a row: its menu lines
              // up with the trigger's right edge so it never runs off it.
            ...
            >
              ...
              <SelectPrimitive.Viewport
                className={cn(
                  ...
                  compact && "min-w-[max(var(--radix-select-trigger-width),208px)] max-w-[280px]",
                    ...
                    >
                      <div className="flex min-w-0 w-full flex-col">
                        ...
                        {option.description && (
                          <span
                            className={cn(
                              "text-xs text-grey-60 dark:text-[#7d7d7d] tracking-normal leading-4",
                              compact ? "font-normal" : "font-mono truncate",
                            )}
                          >
                            {option.description}
                          </span>
