...
const FileDeletionBehaviour: React.FC = () => {
  ...
  const [selectedBehaviour, setSelectedBehaviour] =
    useState<DeletionBehaviour>("upload_only");
  const [originalBehaviour, setOriginalBehaviour] =
    useState<DeletionBehaviour>("upload_only");
  ...
  useEffect(() => {
    const loadCurrentSetting = async () => {
      try {
        const currentBehaviour = (await invoke("get_bucket_policy")) as {
          sync_policy: DeletionBehaviour;
        };
  ...
  const behaviourOptions = [
    {
      ...
      title: "Upload only",
      description:
        "Never download files from the cloud, only upload local files and keep remote backup even after local files are deleted",
      isDefault: true,
    },
    {
      ...
      title: "Remote Backup",
      description:
        "Keep a copy in the cloud. Your deleted local files will stay safe as backups",
      isDefault: false,
    },
    {
      ...
      description:
        "Automatically delete the file from the cloud if it's removed from your local computer",
      isDefault: false,
    },
    {
      ...
      description:
        "Automatically re-download deleted files from the cloud to restore them on your computer",
      isDefault: false,
    },
  ];
    ...
    switch (type) {
      case "upload_only":
        return {
          title: "Upload Only",
          message:
            "This will never download files from the cloud, only upload local files and keep remote backup even after local files are deleted. Do you want to proceed?",
      ...
      case "local_only_deletes":
        return {
          title: "Remote Backup",
          message:
            "This will keep files in the cloud as a backup even after being deleted locally. Do you want to proceed?",
          icon: <Icons.Trash className="size-6 text-white" />,
        };
      case "mirror_local_deletes":
        return {
          title: "Mirror Local Delete",
          message:
            "This will delete all files from the cloud that were deleted locally. Do you want to proceed?",
          icon: <Icons.Trash className="size-6 text-white" />,
        };
      case "restore_from_remote":
        return {
          title: "Restore Local Files",
          message:
            "This will download all files from the cloud storage to the local folder. Do you want to proceed?",
          icon: <Icons.Trash className="size-6 text-white" />,
        };
    }
  };
                ...
                <div className="flex flex-col w-full">
                  <div className="flex items-start justify-between mb-4">
                    <SectionHeader
                      ...
                      learnMoreUrl="https://docs.hippius.com/use/desktop/settings#file-deletion-behavior-settings"
                    />
                    <CardButton
                      className={cn(
