...
const FileCard: React.FC<FileCardProps> = ({ file, state, onClick, actionMenu }) => {
    ...
    useEffect(() => {
        ...
        const cidUrl = `https://get.hippius.network/ipfs/${decodeHexCid(file.cid)}`;

        if (fileType === 'image') {
            const img = document.createElement('img');
            img.onload = () => {
                setThumbnailUrl(cidUrl);
                setIsLoadingThumbnail(false);
            };
    ...
    }, [file.cid, fileType, thumbnailUrl, thumbnailError, loadAttempts, shouldLoadThumbnail, isLoadingThumbnail]);
    ...
    return (
        ...
        >
            {!isDir && (
                <Graphsheet
                    majorCell={{ lineColor: [31, 80, 189, 1.0], lineWidth: 2, cellDim: 50 }}
                    minorCell={{ lineColor: [255, 255, 255, 1.0], lineWidth: 0, cellDim: 0 }}
                            ...
                            <div className='flex items-center sm:justify-center h-[56px] w-[56px] relative'>
                                {isDir ? (
                                    ...
                                    <div className='flex items-center justify-center size-9 bg-primary-50 rounded-[8px] relative'>
                                        <FileTypeIcon fileType={fileType ?? undefined} rawName={file.name} cid={file.cid} size='md' className='text-grey-100' />
                                    </div>
                                )}
                            </div>
                        )}
