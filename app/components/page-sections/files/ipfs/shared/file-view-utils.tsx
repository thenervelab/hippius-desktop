...
export interface FileViewSharedState {
    ...
    isDeleting: boolean;
    handleDelete: () => void;
    handleDownload: (file: FormattedUserIpfsFile, polkadotAddress: string) => void;
    ...
    setContextMenu: (menu: { x: number; y: number; file: FormattedUserIpfsFile } | null) => void;
    handleContextMenu: (e: React.MouseEvent, file: FormattedUserIpfsFile) => void;
}

export function useFileViewShared(props: FileViewSharedProps): FileViewSharedState {
    ...
    } else if ((showUnpinnedDialog && unpinnedFileDetails.length === 0 && unpinnedFiles) || !showUnpinnedDialog) {
    ...
    const handleDownload = useCallback((file: FormattedUserIpfsFile, polkadotAddress: string) => {
    ...
    const handleOpenInExplorer = useCallback(async (file: FormattedUserIpfsFile) => {
        try {
    ...
    const getFileType = useCallback((file: FormattedUserIpfsFile): string | null => {
        const { fileFormat } = getFilePartsFromFileName(file.name);
        return getFileTypeFromExtension(fileFormat || null);
    }, []);

    // Handle context menu events (right-click)
    const handleContextMenu = useCallback((e: React.MouseEvent, file: FormattedUserIpfsFile) => {
        e.preventDefault();
        setContextMenu({
            x: e.clientX,
            y: e.clientY,
