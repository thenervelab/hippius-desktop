...
import { getMinerIdFromBytes } from "@/lib/utils/get-miner-id-from-bytes";
  ...
  if (!isAssigned) {
    return (
      <div className="relative inline-block group overflow-visible">
        ...
        ">
          Please wait a moment while file is being pinned
          <div className="
    ...
    <div className="flex min-w-[200px] lg:flex-wrap gap-2">
      {minerIds.map((id) => {
        ...
        return (
          <span
            key={id}
            className="text-success-20 font-medium text-xs px-2 py-1 rounded-full border border-success-60 bg-success-90"
          >
