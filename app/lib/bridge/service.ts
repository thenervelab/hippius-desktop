 ...
 * Bridge Service
 *
 * Clean rewrite based on the proven Explorer client patterns.
 *
 * Architecture:
 ...
 *
 * Key features:
 * - localStorage transaction persistence for history
 * - Direct polkadot-api + @polkadot-api/sdk-ink usage
 * - Support for both extension signer and local keypair signing
 */
...
export async function bridgeAlphaToHAlpha(
    ...
    try {
        // Build the polkadot-api PolkadotSigner from local keypair.
        // Desktop app always uses local keypair (mnemonic-based signing).
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        let paSigner: any;
        if (request.keypair) {
            ...
            throw new Error('Keypair required for signing in desktop app');
        }
        ensureBittensor();
        ...
        setStep(2, 'active', 'Submitting deposit…');
...
export async function bridgeHAlphaToAlpha(
    ...
    try {
        ...
        if (!result.ok) {
            // Parse the dispatch error for better messaging
            const errorJson = JSON.stringify(result.dispatchError);
            let errorMessage = `Withdrawal failed: ${errorJson}`;

            // Check for Payment error (insufficient balance for fees)
            if (errorJson.includes('Payment') || errorJson.includes('payment')) {
                errorMessage = 'Insufficient balance to pay transaction fees. Please leave at least 0.5 hALPHA in your wallet for fees and minimum balance.';
            } else if (errorJson.includes('InsufficientBalance') || errorJson.includes('BalanceLow')) {
                errorMessage = 'Insufficient balance for withdrawal. Please check your available hALPHA balance.';
            }

            steps[0] = { ...steps[0], state: 'error', detail: errorMessage };
            onStep?.([...steps]);
        ...
        // Improve error message for Payment errors
        if (msg.includes('Payment') || msg.includes('payment') || msg.includes('"type":"Invalid"')) {
            msg = 'Insufficient balance to pay transaction fees. Please leave at least 0.5 hALPHA in your wallet for fees and minimum balance.';
        }
...
export function saveTransactionsToStorage(): void {
    saveToStorage(transactions);
}
...
export async function fetchBridgeDataOnChain(): Promise<BridgeOnChainData> {
    ...
    const hippiusHeight = Number((hippiusBlock as { number: number }).number);
    const bittensorHeight = Number((bittensorBlock as { number: number }).number);
    ...
    type EntryType = { keyArgs: [{ asHex: () => string }]; value: unknown };
    const [depositEntries, withdrawalEntries]: [EntryType[], EntryType[]] = await Promise.all([
        withTimeout(hippiusApi.query.AlphaBridge.Deposits.getEntries(), QUERY_TIMEOUT_MS, 'Deposits') as Promise<EntryType[]>,
        withTimeout(hippiusApi.query.AlphaBridge.WithdrawalRequests.getEntries(), QUERY_TIMEOUT_MS, 'Withdrawals') as Promise<EntryType[]>,
    ]);
