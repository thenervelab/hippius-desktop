...
const UnpinFilesDialog: React.FC<FileDetailsDialogProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchCurrentValidator = async () => {
      try {
        ...
        const validatorData =
          await api.query.ipfsPallet.currentEpochValidator();

        // Parse the Option<(AccountId32,u64)> response
        if (
          validatorData &&
          typeof (validatorData as any).unwrapOrDefault === "function"
        ) {
        ...
        if (api && isConnected) {
          ...
          if (
            typeof (updatedValidatorData as any).unwrapOrDefault === "function"
          ) {
  ...
  useEffect(() => {
    if (!currentBlockNumber || !currentValidatorBlock || !unpinnedFiles.length)
      return;

    const progressMap: Record<string, EpochProgress> = {};

    // Calculate the remaining blocks in the current half-epoch
    const nextEpochPlusOneBlockForEstimate =
      currentValidatorBlock + 2 * BLOCKS_PER_EPOCH;
    const blocksRemaining = Math.max(
      0,
      nextEpochPlusOneBlockForEstimate - currentBlockNumber
    );
    ...
    const percentage = Math.min(
      99,
      Math.floor((blocksPassed / totalEpochBlocks) * 100)
    );
...
export default UnpinFilesDialog;
