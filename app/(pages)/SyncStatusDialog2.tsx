...
const SyncFileItem = memo<SyncFileItemProps>(
  function SyncFileItem({
    ...
    const isEncryptingOrDecrypting =
      file.status === "encrypting" || file.status === "decrypting";
    const isFileInProgress =
      file.status === "inProgress" || isEncryptingOrDecrypting;
    ...
    return (
      <div className="mb-4 last:mb-0" data-file-item data-testid="file-item">
              ...
              <div className="flex items-center flex-shrink-0 ml-2">
                {isFileDeleted ? (
                  ...
                  isEncryptingOrDecrypting ? (
                    <span className="text-xs text-primary-50">
                      ...
                      <span className="inline-flex w-[0.875rem]">
                        <span className="animate-[dotPulse_1.4s_ease-in-out_infinite]">
                          .
                        </span>
                        <span className="animate-[dotPulse_1.4s_0.2s_ease-in-out_infinite]">
                          .
                        </span>
                        <span className="animate-[dotPulse_1.4s_0.4s_ease-in-out_infinite]">
                          .
                        </span>
                      </span>
                    </span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-grey-50 min-w-[2rem] text-right">
                      0%
                    </span>
                  </div>
                )}
...
interface SyncStatusDialog2Props {
  snapshot: SyncSnapshot;
  open: boolean;
  onClose?: () => void;
}

const SyncStatusDialog2: React.FC<SyncStatusDialog2Props> = ({
  snapshot,
  open,
  onClose,
}) => {
  ...
  if (snapshot.totalFiles === 0 && !isInProgress && !isRetrying && !isCompleted)
    return null;
    ...
    >
      ...
      <div
        className={cn(
          ...
          isExpanded ? "rounded-t-[0.5rem]" : "rounded-[0.5rem]",
        )}
          ...
          >
            ...
            {!isExpanded && (
              <span
                className={cn(
                  "text-sm mr-2",
                  hasFailed ? "text-error-50" : "text-grey-40",
                )}
              >
                {isUnhealthy
                  ? "Disconnected"
      ...
      >
        <div
          className="bg-grey-100 border border-grey-80 rounded-b-[0.5rem] flex flex-col"
          style={{ width: W_EXPANDED, maxHeight: `${BODY_MAX_HEIGHT_REM}rem` }}
        >
          {/* Status banner */}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {(() => {
                ...
                const variantClasses: Record<string, string> = {
                  ...
                  success:
                    "bg-success-100/40 border-success-80 text-success-40",
                  progress:
                    "bg-primary-100/40 border-primary-80 text-primary-40",
                };
                ...
                const classes =
                  variantClasses[snapshot.statusVariant] ||
                  variantClasses.progress;
                return (
                  <div
                    className={cn("w-fit px-2 py-0.5 border rounded", classes)}
                  >
              ...
              <div className="flex items-center justify-between mt-1">
                <div className="flex items-center gap-2">
                  ...
                  (snapshot.intentTotalBytes ?? 0) > 0 ? (
                    <span className="text-[0.625rem] text-grey-50">
                      {formatBytes(snapshot.intentCompletedBytes ?? 0)} of{" "}
                      {formatBytes(snapshot.intentTotalBytes ?? 0)}
                    </span>
                  ) : snapshot.bytesExpected > 0 ? (
                    <span className="text-[0.625rem] text-grey-50">
                      {formatBytes(snapshot.progressBytes)} /{" "}
                      {formatBytes(snapshot.bytesExpected)}
                    </span>
                  ) : null}
                  {effectiveInProgress &&
                    speedBytesPerSec !== null &&
...
export default SyncStatusDialog2;
