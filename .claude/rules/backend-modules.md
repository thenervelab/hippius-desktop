...
- **`updates.rs`** — In-app updater aimed at this build's own channel. Linux ships `.deb`; `install_update` and `switch_release_channel` refuse in-place install with `AppError::Validation` (Tauri cannot apply a `.deb`). macOS/Windows still call `download_and_install`. Pinned by `tests/linux_deb_update_wiring.rs`.
