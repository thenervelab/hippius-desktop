...
- **`billing/`** — `charts.rs` (chart data formatting plus shared helpers — `parse_timestamp_to_date`/`parse_timestamp_to_datetime`, `planck_str_to_credits`, `format_balance`/`format_bytes`, `range_start`; plan-card `storage_display` rounds TB labels so 3/150/450 credits read as 1/50/150 TB — `storage_gb` stays the binary-search result and is what `storage_overview` prices capacity from), `credit_balance.rs`, `drive_credits.rs` (cumulative drive credit *spend*), `drive_storage.rs`, `credits.rs`, `queries.rs`, `subscriptions.rs`, `storage_overview.rs`, `eligibility.rs`, `account_cache.rs`, plus the two Drive-plan modules below.

  **`drive_plans.rs`** — the `/api/drive/...` rail: plan catalogue, the account's subscription, subscribe / change / cancel, Stripe card checkout and its intent, and history. Every command forwards the API's JSON as-is; the shapes are owned by the frontend's `app/lib/types/drive-plans.ts`, the same file the console uses, so the two clients cannot drift. This is a **separate rail** from the Stripe credit top-ups in `subscriptions.rs`.

  **`drive_quota.rs`** — "would hcfs-server accept this write?", the gate `require_eligible` runs for every Drive write. It asks the server's own `POST /can_upload` pre-flight (`hcfs-shared::network::CanUploadRequest`, bearer = the hcfs API token from `get_api_token`, base URL from `hcfs_config` or `pick_fastest` for the auto-detect sentinel) and **does not re-derive the plan itself**. The server's rule is grant-only — a plan covers its allowance, credits pay beyond it, only a double denial refuses — and which plan an account is on (drive rail, legacy Stripe subscription, free tier) is the Drive backend's business; a local model of that drifted three times in one week and refused paying accounts the server accepted. Every server denial (`drive_quota_exceeded`, `drive_not_entitled`, `zero_balance`, `insufficient_balance…`) is `NotReady(StorageLimitReached)` → the plans dialog, never the credits dialog; the slug is logged. It **fails open** on anything short of a verdict — no token, transport failure, 5xx, unparseable body, or the server's own `Failed to fetch billing balance` outage string — because hcfs-server gates the write itself.

  `is_drive_storage()` is `!matches!(self, VmCreation)`, so it also catches **`Sharing`**. A share re-encrypts the file under a per-share key and uploads that copy, which the server bills like `/upload`, so every share path gates on the copy's real size once it is known (`create_share_inner`, `share_external_file`, `mint_remote_share_at` — pinned by `every_share_upload_is_gated_on_its_plaintext_size` / `no_share_path_gates_with_zero_bytes`). Folder shares upload nothing and have no gate.

  `tests/drive_quota_enforcement.rs` covers the gate end to end against a mocked hcfs-server + Drive API + indexer, including the case that shipped: Drive API says no plan, indexer says 60 GiB, server says yes → allowed. The credit-era `eligibility_enforcement.rs` cannot: its mock serves no `/can_upload`, so every Drive action there falls open by design.
...
**Drive writes are no longer among them.** `require_eligible` and `check_action_eligibility_inner` both return early for `action.is_drive_storage()`, routing to `drive_quota::check_drive_quota` — storage is sold as a plan, so the question is whether the bytes fit the allowance, not whether a balance covers them. A refusal is `NotReady(StorageLimitReached)`, not `InsufficientCredits`. The byte-priced credit layer below is therefore unreachable for uploads; it stays for the record and is pinned by `tests/hippius_eligibility_pricing.rs`, which now asserts the inverse (an upload's outcome does not depend on the balance, at any size). VM creation is the only remaining credit-gated action.
