...
- **`billing/`** — `charts.rs` (chart data formatting plus shared helpers — `parse_timestamp_to_date`/`parse_timestamp_to_datetime`, `planck_str_to_credits`, `format_balance`/`format_bytes`, `range_start`), `credit_balance.rs`, `drive_credits.rs` (cumulative drive credit *spend*), `drive_storage.rs`, `credits.rs`, `queries.rs`, `subscriptions.rs`, `storage_overview.rs`, `eligibility.rs`, `account_cache.rs`.
...
- **`updates.rs`** — In-app updater aimed at this build's own channel. Installs in-place on **every** platform: the plugin resolves the manifest key for its own bundle (`linux-x86_64-deb` here) and applies a `.deb` with `dpkg -i` behind pkexec. **Rust owns every sentence the update dialog shows** — the plugin's `Display` is diagnostics ("Failed to install package", a reqwest chain) and the dialog renders `message` verbatim, so `install_failure`/`check_failure` classify the error, log the original, and return copy naming the channel's release page. The manual-install wording is keyed on `bundle_type()`, never `target_os` — a `.deb` and an AppImage are both Linux. Pinned by `tests/updater_install_paths.rs`.
- **`notifications/`** — Notification management commands. Sync-complete list titles are derived in Rust (`list_title`) from the `release_notes` JSON (one file → `Synced {basename}`; several → `Synced n files`); the bell and the notifications page render `title_text` and nothing else, so the title is the whole of what a user reads about a finished cycle. Two rules keep it from lying: **only uploads and downloads lend their name** (`Synced report.pdf` for a file the cycle DELETED reads as a successful upload of a file that is gone, so a delete-only cycle keeps the generic title), and **the count comes from the `fileCount` IPC arg, never from the list length** — the list is capped at `MAX_NOTIFICATION_FILES` and again by the hook's own buffer, so counting it titles a 5 000-file sync `Synced 200 files`. The single-name arm additionally requires the count and the list to agree on exactly one file. Error / restored titles stay fixed.
