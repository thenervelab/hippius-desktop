...
- **`billing/`** — `charts.rs` (chart data formatting plus shared helpers — `parse_timestamp_to_date`/`parse_timestamp_to_datetime`, `planck_str_to_credits`, `format_balance`/`format_bytes`, `range_start`; plan-card `storage_display` rounds TB labels so 3/150/450 credits read as 1/50/150 TB — `storage_gb` stays the binary-search result and is what `storage_overview` prices capacity from), `credit_balance.rs`, `drive_credits.rs` (cumulative drive credit *spend*), `drive_storage.rs`, `credits.rs`, `queries.rs`, `subscriptions.rs`, `storage_overview.rs`, `eligibility.rs`, `account_cache.rs`, plus the two Drive-plan modules below.
  ...
  **`drive_quota.rs`** — "would this write fit under the account's plan?", and the gate `require_eligible` runs for every Drive write. Storage is sold as a plan, so a write is judged by the plan allowance against indexer usage plus the incoming bytes — **not** by a credit balance (VM creation is genuinely credit-priced and keeps its own gate). `active: false` is NOT "no allowance": every account has the free tier, so the allowance falls back to the catalogue's free SKU (`free_plan_bytes`, shared with `storage_overview` so the card and the refusal quote the same field). Treating that as unknown is what let the desktop accept uploads the console refused. It **fails open** — an unreadable allowance or usage lets the write through and leaves hcfs-server's own gate as the backstop — so this check can never refuse a write the server would accept.

  `is_drive_storage()` is `!matches!(self, VmCreation)`, so it also catches **`Sharing`** — minting a share link passes `0` incoming bytes but still runs the allowance check, and an account ALREADY past its allowance is refused a share. That is intended (an over-allowance account is frozen out of Drive actions until it upgrades), but it means `thresholds::SHARING` decides nothing any more, and the FE needs a `"sharing"` reason so the refusal reaches the plans dialog rather than a bare toast.

  The comparison lives alone in `QuotaVerdict::decide` so the tests exercise the real rule; they previously asserted against a copy of the expression written in the test module, which stayed green whatever the production line did. Same trap as any "pure helper" test — re-implementing the logic to test it tests nothing. `tests/drive_quota_enforcement.rs` covers the refusal end to end against a mocked Drive API + indexer, which the credit-era `eligibility_enforcement.rs` cannot: its mock serves no drive endpoints, so every Drive action there falls open by design.
