...
- **`notifications/`** — Notification management commands. Sync-complete list titles are derived in Rust (`list_title`) from `release_notes` JSON (one file → `Synced {basename}`; several → `Synced n files`); the bell renders `title_text`, not the description. Error / restored titles stay fixed.
