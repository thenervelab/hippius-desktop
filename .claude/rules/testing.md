...
- Team chat: `chat_sign_in_mock.rs` (mock-axum issuer + homeserver standing in for MAS: drives the REAL `chat::sign_in` flow — discovery fallback, auth metadata, dynamic registration, loopback listener, PKCE code exchange with the SAME `redirect_uri` as the authorize leg, `/whoami` device-binding refusal, refresh rotation + `invalid_grant`, cancelled flow frees the port; no keyring, no `AppHandle`). The 4S derivation KATs (console `KEYS.md` §6), the PKCE RFC 7636 Appendix B vector, the loopback-redirect shape MAS accepts, the keyring record wire shape, and the notification policy table are in-module unit tests under `src/chat/`.
- Team chat, frontend (`pnpm test`): `app/components/chat/__tests__/ChatProvider.test.tsx` mounts the REAL provider + REAL encryption bootstrap against the fake homeserver (`app/lib/chat/crypto/testing/fake-world.ts`, the console's test double ported to take key BYTES — what Rust derives — instead of a mnemonic; coverage-excluded), with Rust replaced by `makeTauriMock` and only `startChatClient`/`signOutChat` stubbed. Pins: keyring-unavailable → `unavailable` (not signed-out); corrupt session → session-less error; first-sync failure keeps the session and stops the client; a fresh device on an account with published cross-signing runs `bootstrapCrossSigning(reset=true)` with the key from `chat_derive_secret_storage_key` and publishes it under Rust's key id; a complete device never asks Rust for the key; sign-in opens Rust's authorize URL in the system browser and boots the persisted session; cancel calls `chat_cancel_sign_in` and ignores the late rejection; sign-out goes through `signOutChat`. `ChatRoute.test.tsx` pins the runtime gate (unknown → nothing, disabled → `router.replace("/")`) and the state→screen map, notably that `unavailable` offers Retry ONLY. `app/lib/chat/crypto/__tests__/bootstrap.test.ts` is the console's 38-case bootstrap suite ported 1:1; `client.test.ts` pins the pure helpers: `applyRefreshedTokens` (keeps the old refresh token when the issuer did not rotate it), `isStoreAccountMismatch`, `initialSyncError` wording and the sliding-sync list shapes (the live token-refresh wiring SDK ← `chat_refresh_tokens` is exercised only against a real homeserver); `stores.test.ts` pins the sha256 store names (frozen digest, no user/device id in clear) and the delete-others sweep; `secret-storage-keys.test.ts` pins the 32-byte decode and the MAC-checked `chatCryptoCallbacks`. Desktop surfaces (F4): `app/lib/chat/__tests__/notifications.test.ts` is the classifier table (own/echo/non-message/undecrypted/redacted/history/push-rules-silent → `null`; DM detection; `isMention` from the highlight tweak; a plain channel message IS reported so Rust — not the webview — applies the mentions-only rule); `rooms.test.ts` pins `attentionCount`; `hooks/__tests__/useChatNotifications.test.tsx` (decrypt wait, back-pagination ignored, open room read at event time, unsubscribe) and `useChatUnreadBadge.test.tsx` (IPC only on change, `0` when the client goes) drive the hooks with an `EventEmitter` client and `makeTauriMock`; `ChatHost.test.tsx` pins the provider-always-mounted-and-gated contract and `openRoomFor`; `ChatProvider.test.tsx` pins `active=false` → nothing touched, flip → boot. Rust: `chat::notify::tests` (policy table, `window_title`, the `APP_WINDOW_TITLE` ↔ `tauri.conf.json` pin) and `tests/chat_unread_wiring.rs` (logout resets the badge AFTER the auth clear; the five `chat::notify` commands are registered).
- Remote writes: `remote_write_real_backend.rs` (`#[ignore]` live lane: uploads a file into a folder with NO local sync root, downloads it back and asserts the bytes are IDENTICAL, then renames it and asserts the listing moved rather than duplicated; a second test renames a FOLDER and asserts, by browsing, that its nested contents AND an empty subfolder moved with it, that the old folder is no longer listed (the `folder_entries` half), and that a lookalike sibling (`Trip` vs `Trip Photos`) was untouched; needs `HCFS_DESKTOP_E2E_SERVER_URL` + `HCFS_DESKTOP_E2E_BEARER` + `HCFS_DESKTOP_E2E_SS58`, and the bearer must be a **user** bearer for the same reason folder-shares needs one). This is the only layer that can catch a wrong `Manifest` / `BatchRenameRequest`: `remote_upload` and `remote_rename` assemble those by hand from public `hcfs-client` primitives, and a subtly wrong one is accepted by every hermetic test — it uploads happily and surfaces months later as a file that will not decrypt. The hermetic unit tests cover the path-string normalisation only.
