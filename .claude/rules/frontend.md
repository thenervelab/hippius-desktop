...
- **FE is a pure projection**: `useStorageOverview` (poll cadence mirrors `useDriveStorageStats`; `STORAGE_OVERVIEW_QUERY_KEY` **and** `DRIVE_STORAGE_STATS_QUERY_KEY` are invalidated together by `useSyncEvents` on **every** `hcfs_sync_completed`, including zero-file cycles — empty indexer data is success + 0 bytes, so a no-op cycle still has to re-probe, and since both keys read the SAME indexer row, refreshing only one is how the two surfaces come to disagree about one number) + the unit-tested resolvers in `storage-overview/storageOverviewState.ts` — `getStorageOverviewView` (skeleton → error → no-plan → usage), `getUsedBytesDisplay` (pending vs indexer bytes — never inferred from `usedBytes === 0`), `getPlanView` (skeleton → plan → credits → none, shared by the plan card AND the chip), `getUsageTone` (brand < 80% ≤ amber < 95% ≤ red), `getCapacitySourceLabel` (a credits-derived total is always labelled "Based on your credit balance", never as a plan). **Render `usedDisplay` / `totalDisplay` / `freeDisplay` from Rust** — do not `formatBytes` the raw counts on this card (H-109). Credits capacity is the buyable SKU, not used+buyable, so a 15-credit balance is 5.00 TB on the card and the chip. Plan chip/card render `plan.storageDisplay`, never `formatBytes(plan.storageBytes)`. **The skeleton latches to first settle** so the chip/cards never flash "No active plan" while the decision is merely loading.
