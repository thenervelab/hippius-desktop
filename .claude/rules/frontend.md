...
- `SHARED_DRIVES_ENABLED = enabledFrom("beta")` — on in beta and staging, off in production while the feature is proven against a live fleet. See the shares rules file.
...
The home page's two chart cards were removed by product decision in favor of TWO small mobile-style cards: a **storage card** (used vs effective capacity, progress bar — `app/components/page-sections/home/storage-overview/`) and a **plan card** (subscribed plan / free plan + upgrade CTA — `.../home/plan-overview/`), plus the **shared `ui/plan-chip`** component that renders the "Active Plan"/"Free Plan" cell in BOTH page headers (`home/PageHeader.tsx` and the global `ui/page-header` used by Files / VM / Notifications) — neither header carries its own plan logic anymore.
...
- **Only the Storage/Plan row is width-capped; the rest of the home page is full-bleed.** `max-w-[960px]` on that grid alone, never on the page wrapper — a page-wide capped column narrows Recent Files too, and that table spends real width on filenames, where a card holding a figure and a button reads as an empty banner past ~700px. Uncapped and page-wide-capped have both shipped and both were wrong. Pinned by `home/__tests__/homeLayout.test.ts` (exactly one cap, on the card row, none on the wrapper or the files block).
