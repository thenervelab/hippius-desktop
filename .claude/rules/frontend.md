...
- **FE is a pure projection**: `useStorageOverview` (poll cadence mirrors `useDriveStorageStats`; `STORAGE_OVERVIEW_QUERY_KEY` is invalidated by `useSyncEvents` on **every** `hcfs_sync_completed`, including zero-file cycles — empty indexer data is success + 0 bytes, so a no-op cycle still has to re-probe) + the unit-tested resolvers in `storage-overview/storageOverviewState.ts` — `getStorageOverviewView` (skeleton → error → no-plan → usage), `getUsedBytesDisplay` (pending vs indexer bytes — never inferred from `usedBytes === 0`), `getPlanView` (skeleton → plan → credits → none, shared by the plan card AND the chip), `getUsageTone` (brand < 80% ≤ amber < 95% ≤ red), `getCapacitySourceLabel` (a credits-derived total is always labelled "Based on your credit balance", never as a plan). **The skeleton latches to first settle** so the chip/cards never flash "No active plan" while the decision is merely loading.
