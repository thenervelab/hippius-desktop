...
- **`sync/`** — Core sync engine (16 submodules): `lifecycle.rs` (init, auto-init, teardown, progress handler setup), `control.rs` (start/stop/pause drive commands), `files.rs` (file listing, recent files, user files with parallel folder listing), `folders.rs` (folder CRUD), `paths.rs` (sync path DB operations), `config.rs` (HCFS config loading), `progress.rs` (in-memory session/file progress tracking), `status.rs` (sync status queries), `events.rs` (event name constants and payload structs), `logic.rs` (pure I/O-free helpers: snapshot throttling), `migration.rs` (S3→HCFS migration: check, start, poll, complete_migration_transition), `mnemonic.rs` (mnemonic resolution for sync), `remote.rs` (remote folder operations), `recent_uploads.rs` (`get_recent_uploads` — fetches account-wide "last uploads" from the HCFS server's `/search_files` endpoint sorted by `created_at desc`, maps hits onto `UserFileEntry`; one-shot Arion call via `region::resolve_base_url` + `tokens::get_api_token`, mirroring `migration::fetch_migration_summary`), `selective.rs` (selective sync), `device.rs` (device name), `tauri_bridge.rs` (OnceLock-based event dispatch to Tauri).
...
**Tray popover panel**: The native tray menu is replaced by a borderless, transparent, always-on-top webview window (label `tray-panel`, route `/tray-panel`) anchored to the tray icon, opened on **left-click**. **Right-click shows a small native context menu** (Open Files / Open Virtual Machines / Quit Hippius) built by `buildTrayContextMenu` in `useTraySync.ts` and attached to the icon with `showMenuOnLeftClick: false` so it never hijacks the left click; "Open Hippius" is omitted because the popover already has that button. The context menu's Open Files/VM items are tracked in the module-level `openFilesItem`/`openVmItem` so the existing login-status watcher enables/disables them. Flow: the tray icon's `action` callback in `useTraySync.ts` (`handleTrayClick`) forwards the icon's screen `rect` to the Rust `toggle_tray_panel` command (`src-tauri/src/tray/panel.rs`); Rust resolves the monitor under the icon, computes placement via the pure `tray::geometry::compute_panel_position` (drop below when the icon is in the top half of its work area — macOS menu bar / top taskbar — else rise above; centre on the icon and clamp on-screen; all physical px), then positions+shows the reused window (or hides it if already visible — a toggle). **Linux fallback**: Tauri's `tray-icon` crate fires no left-click `action` event on Linux, so the icon there is built with `showMenuOnLeftClick: true` and the context menu leads with an **"Open Hippius"** item (added by `buildTrayContextMenu` only when `isLinuxPlatform`, a module mirror set from `get_platform_info` during `useTrayInit`). That item calls `toggle_tray_panel` with **`rect: null`**; `toggle_tray_panel` takes `Option<TrayIconRect>` and, on `None`, uses a **deterministic top-right anchor** (`fallback_anchor` → a zero-size `Rect` at the primary monitor work-area's top-right corner, where GNOME/most-Linux trays live) since no icon bounds exist — the same geometry then drops + clamps the panel there. (An earlier cursor-based anchor proved unreliable: under Wayland `set_position` is a no-op and the window landed top-left.) `handleTrayClick` stays attached but is a harmless no-op on Linux. **Opaque card off macOS**: the frosted look needs native vibrancy, which only macOS has here, so the FE (`page.tsx` `cardSurface`, gated on `get_platform_info`) paints a **translucent** card only on macOS and an **opaque** card + hairline border on Linux/Windows — a 0.7-alpha card over the transparent window otherwise shows the desktop straight through (the "very transparent" popover reported on Linux). The native window shadow is likewise macOS-only (`.shadow(cfg!(target_os = "macos"))`); the vibrancy `effects` call is left unconditional because Tauri no-ops it off macOS. Key details:
...
- **Known limitations / follow-ups**: (1) on **Linux** the popover opens from the native menu's "Open Hippius" item (left-click shows the menu) and uses a fixed top-right anchor rather than the icon, because the `tray-icon` crate fires no left-click `action` event and exposes no icon bounds there (and under Wayland the app cannot self-position at all) — see the Linux fallback above; (2) on macOS a plain always-on-top window activates the app on click (an `NSPanel` non-activating window is the eventual fix); (3) `useTraySync.ts` still builds the old **full** in-memory `Menu`/submenu objects (sync-activity rows, drive submenu) which remain **unattached** — only the small `buildTrayContextMenu` menu is attached for right-click; the full menu is kept to preserve the icon-state machinery and its tests, and deleting that now-dead code is a tracked follow-up.
