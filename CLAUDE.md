...
- **Transparency**: requires `app.macOSPrivateApi: true` in `tauri.conf.json` **and** the `macos-private-api` Cargo feature on `tauri`. The `tray-panel` window label is added to the `default` capability's `windows` list. The native window shadow is disabled (`.shadow(false)`) because on a transparent window it is computed from the rectangular bounds (a dark rectangle around the rounded card); the card's CSS `box-shadow` provides the shadow instead.
