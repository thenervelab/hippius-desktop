...
- **CI**: the release workflows write the file from the **`TAURI_ENV_FILE`** repository secret through `scripts/write-tauri-env.sh`. `ci.yml` writes the same file; it only needs it to exist for `cargo check`.
