...
`src-tauri/.env` is gitignored, bundled as a Tauri resource, and loaded at runtime by dotenvy (`main.rs`). It must **exist** — bundling a missing resource fails the build — but everything in it is optional, so an empty file is a working one. `pnpm setup:env` (`scripts/dev-env.mjs`) creates it from `.env.example`; `pnpm tauri:dev` and `pnpm tauri:build` run the same script first, so a fresh clone needs no setup step.

- **`.env.example`** is the committed template, listing the two optional vars the app reads: `HIPPIUS_INDEXER_URL` and `HIPPIUS_CONSOLE_BASE_URL`.
- **CI**: the release workflows write the file from the **`TAURI_ENV_FILE`** repository secret through `scripts/write-tauri-env.sh`, which **allowlists** — only the two vars above survive into the bundle. That matters because the secret is a hand-maintained file that still carries `INDEXER_API_KEY`, and anything written here is extractable from the installer. Filtering on the way in means a stale credential in the secret cannot ship, and neither can the next one added by mistake. `ci.yml` writes the same file; it only needs it to exist for `cargo check`.
...
**This used to be a required credential, and the history explains three guards you will still find traces of.** `.env` carried `INDEXER_API_KEY`; without it the indexer answered 401, `api/indexer.rs::IndexerClient` failed, and every indexer-backed surface rendered a **confident zero instead of an error** — a build with no key looked healthy and the symptom read as a data bug. So `dev-env.mjs` resolved the key by reading it back out of an installed Hippius build, which worked because the same key shipped inside every release, and `write-tauri-env.sh` failed the build without one.

That key is gone. The indexer authenticates the **logged-in user** instead: `IndexerClient::for_session` mints the session account from `AppState::current_session_account` and sends that account's bearer token, and the indexer resolves it to an SS58 and answers only for that account on the routes carrying account records (design: hippius-indexer `docs/plans/2026-09-11-indexer-user-token-auth-design.md`). Two routes are deliberately exempt from that binding — `/system-account-balance` and `/balance-transfers` serve public chain state, and the wallet page reads them for the header-selected wallet rather than the login identity. The "error, never a zero" rule survives the change: no session is an `AppError::Auth`, not an empty result.
...
- **Never read `useUserCredits` for an eligibility decision** — call `check_action_eligibility`. Every gated IPC also enforces `require_eligible` itself — as its first line, or, for a share, at the point the uploaded copy's size is known (pinned by `every_share_upload_is_gated_on_its_plaintext_size`).
- **A Drive write is gated by hcfs-server's own `/can_upload` pre-flight, never by credits and never by desktop-side allowance math** — `require_eligible` routes `is_drive_storage()` actions (shares included, sized by the copy they upload) to `drive_quota::check_drive_quota`, which asks the server and refuses with `NotReady(StorageLimitReached)`; anything short of a verdict falls open to the server's gate on the write. Credits gate VM creation only.
