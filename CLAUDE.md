...
- **Three long-lived branches**, promoted by squash at each hop: `staging` (internal) → `beta` (public opt-in) → `main` (production). `staging` → `tauri-staging.yml` → `v<version>-staging` prerelease; `beta` → `tauri-beta.yml` → `v<version>` prerelease; `main` → `tauri-build.yml` → `v<version>` production release. Only `main` is ever marked latest. All three publish **on push** — there is no build-without-releasing mode. `main` requires linear history, so every promotion is a **squash**. The older `redesign` / `redesign-preview` / `sync-engine` branches and the `tauri-dev.yml` lane are retired.
...
- **Staging has no auto-update** (`ReleaseChannel::Staging::manifest_url()` is `None`) and needs no `latest.json` job. Beta does: `publish-manifest` republishes the manifest to a fixed `beta-channel` release tag, because every beta release carries its own version tag and `releases/latest` never resolves to a prerelease.
