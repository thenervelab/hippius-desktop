...
- **Staging has no auto-update** (`ReleaseChannel::Staging::manifest_url()` is `None`) and needs no `latest.json` job. Beta does: `publish-manifest` republishes the manifest to a fixed `beta` release tag, because every beta release carries its own version tag and `releases/latest` never resolves to a prerelease.
