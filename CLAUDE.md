...
- **Known limitations / follow-ups**: (1) on **Linux** the popover opens from the native menu's "Open Hippius" item (left-click shows the menu) and anchors to the cursor rather than the icon, because the `tray-icon` crate fires no left-click `action` event and exposes no icon bounds there — see the Linux fallback above; (2) on macOS a plain always-on-top window activates the app on click (an `NSPanel` non-activating window is the eventual fix); (3) `useTraySync.ts` still builds the old **full** in-memory `Menu`/submenu objects (sync-activity rows, drive submenu) which remain **unattached** — only the small `buildTrayContextMenu` menu is attached for right-click; the full menu is kept to preserve the icon-state machinery and its tests, and deleting that now-dead code is a tracked follow-up.
