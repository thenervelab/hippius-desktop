...
**Migration folder picker**: The `MigrationPromptDialog` includes a folder picker pre-populated with the default migration path (`~/Hippius-Migration-YYYY-MM-DD`, falling back to `~/Documents/Hippius-Migration-YYYY-MM-DD` only when home is unresolvable). The chosen path is stored in `migrationCheckAtom.syncPath` and passed to `complete_migration_transition` as `custom_sync_path`. If not provided, falls back to the auto-generated default. Home is preferred over Documents because macOS TCC gates Documents and only persists the permission grant against a stable code identity (Developer ID) — our ad-hoc-signed builds re-prompt on every access until signed + notarized, so keeping the default outside Documents removes the friction.
