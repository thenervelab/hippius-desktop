...
- **Syncing no longer fills up your disk.** While preparing an upload, Hippius writes a
  temporary encrypted copy of the file. Copies left behind by uploads that were
  interrupted — by a dropped connection, a pause, or quitting the app — could pile up
  until the drive ran out of space. Hippius now clears them out on launch and when you
  remove a synced folder, and keeps the total bounded from here on. Existing users get
  the space back automatically on the next launch; nothing to run or delete by hand.
